...
export default function XlsxDialog({ filekey, open, close, setError }: FileViewerProps) {
  ...
  useEffect(() => {
    ...
    fetch(fileLink, { signal: ac.signal })
      .then(res => res.blob()).then(blob => xlsxPreview.xlsx2Html(blob)).then(setHtml, err => {
